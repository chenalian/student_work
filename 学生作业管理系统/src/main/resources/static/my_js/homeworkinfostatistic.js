...
layui.use(['table','jquery'], function(){
    ...
    var infotable=table.render({
        ...
        ,cols: [[
            ...
            ,{fixed: 'tex', title:'操作', toolbar: '#barDemo1', width:260,fixed: 'right'}
        ]]
        ,page: true
    });
    //头工具栏事件
    ...
    table.on('tool(test1)', function(obj){
        ...
        var info=data.text;
        var id=data.id;
        if(obj.event==='text')// 关键字统计
        {
            xadmin.open('关键字统计','/teacher/keywords?id='+id,600,500);
        }
        else if(obj.event==='score'){// 分数段统计
            xadmin.open('分数段统计','/teacher/scorestatictis?id='+id,1000,600);
        }
    });
});
